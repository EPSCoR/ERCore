...
// JSLint options:
/*jslint forin: true */
/*global document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQueryNew, $ */
...
/**
 * Set or get an attribute or an object of attributes. Can't use jQueryNew attr because
 ...
 * Highcharts reverts to the built-in jQueryNew adapter.
 */
if (globalAdapter && globalAdapter.init) {
	globalAdapter.init();
} 
if (!globalAdapter && win.jQueryNew) {
	var jQ = jQueryNew;
	
	/**
	 * Utility for iterating over an array. Parameters are reversed compared to jQueryNew.
	 * @param {Array} arr
	 * @param {Function} fn
	 */
	...
	removeEvent = function(el, eventType, handler) {
		// workaround for jQueryNew issue with unbinding custom events:
		// http://forum.jQueryNew.com/topic/javascript-error-when-unbinding-a-custom-event-using-jQueryNew-1-4-2
	...
	fireEvent = function(el, type, eventArguments, defaultFunction) {
		...
		// Prevent jQueryNew from triggering the object method that is named the
		// same as the event. For example, if the event is 'select', jQueryNew
	 ...
	 * @param {Object} el
	 * @param {Object} params
	 * @param {Object} options jQueryNew-like animation options: duration, easing, callback
	 */
	animate = function (el, params, options) {
		var $el = jQ(el);
		if (params.d) {
			el.toD = params.d; // keep the array form for paths, used in jQ.fx.step.d
			params.d = 1; // because in jQueryNew, animating to an array has a different meaning
		}
	...
	var oldStepDefault = jQueryNew.fx.step._default, 
		oldCur = jQueryNew.fx.prototype.cur;
...
SVGElement.prototype = {
	 ...
	 * @param {Object} params
	 * @param {Number} options The same options as in jQueryNew animation
	...
	attr: function(hash, val) {
		...
		// setter
			...
			for (key in hash) {
				...
				// jQueryNew animate changes case
				if (key == 'strokeWidth') {
					key = 'stroke-width';
				}
...
Series.prototype = {
	...
	remove: function(redraw, animation) {
		...
		if (!series.isRemoving) {  /* prevent triggering native event in jQueryNew
